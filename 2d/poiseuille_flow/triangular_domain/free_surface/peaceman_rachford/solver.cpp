...
using namespace PoiseuilleFlow::TriangularDomain::FreeSurface::PeacemanRachford;
...
void Solver::solve() {
    ...
    while (t <= endTime) {
        ...
        double wbt = cos(M_PI*x)*cos(M_PI*y)*exp(-2*M_PI*M_PI*nu*t);
        double w1bt = cos(M_PI*x)*cos(M_PI*y)*exp(-2*M_PI*M_PI*nu*(t + dt));

        vt[0] = (wbt + w1bt)/2;

        for (int i = 1; i < nx - 1; i++) {
            vt[i] = (1 - ry)*w[i][ny-1] + ry*w[i][ny-2];
        }

        x = x0 + (nx - 1)*dx;

        wbt = cos(M_PI*x)*cos(M_PI*y)*exp(-2*M_PI*M_PI*nu*t);
        w1bt = cos(M_PI*x)*cos(M_PI*y)*exp(-2*M_PI*M_PI*nu*(t + dt));
        
        vt[nx-1] = (wbt + w1bt)/2;

        Tridiagonal::solve(-0.5*rx, 0, 1 + rx, 1, 1, -0.5*rx, 0, vt, wt);
            ...
            for (int i = 0; i < nx; i++) {
                ...
                if (isDomain) {
                    if (isBoundary) {
                        ...
                        double wb = cos(M_PI*x)*cos(M_PI*y)*exp(-2*M_PI*M_PI*nu*t);
                        double w1b = cos(M_PI*x)*cos(M_PI*y)*exp(-2*M_PI*M_PI*nu*(t + dt));
        ...
        for (int i = 0; i < nx; i++) {
            ...
            for (int j = 0; j < ny; j++) {
                ...
                if (isDomain) {
                    if (isBoundary && (j != ny - 1 || i == 0 || i == nx - 1)) {
                        double x = x0 + i*dx;
                        double y = y0 + j*dy;

                        vy.push_back(cos(M_PI*x)*cos(M_PI*y)*exp(-2*M_PI*M_PI*nu*(t + dt)));
                    } else {
            ...
            Tridiagonal::solve(-0.5*ry, -ry, 1 + ry, 1, 1 + ry, -0.5*ry, 0, vy, wy);
