...
using namespace PoiseuilleFlow::TriangularDomain::CranckNicolson;
...
void Solver::calculateResidualElements(
    ...
    for (int j = 0; j < ny; j++) {
        for (int i = 0; i < nx; i++) {
            ...
            if (isDomain) {                
                if(isBoundary) {
                    d[k] = cos(M_PI*x)*sin(M_PI*y)*exp(-2*M_PI*M_PI*nu*t);
                } else {
                    ...
                    if (isDomain) {
                        if (isBoundary) {
                            wl = cos(M_PI*(x - dx))*sin(M_PI*y)*exp(-2*M_PI*M_PI*nu*t);
                            d[k] += rx*cos(M_PI*(x - dx))*sin(M_PI*y)*exp(-2*M_PI*M_PI*nu*(t + dt))/2;
                        } else {
                            wl = w[i - 1 + j*nx];
                        }
                    }
                    ...
                    if (isDomain) {
                        if (isBoundary) {
                            wr = cos(M_PI*(x + dx))*sin(M_PI*y)*exp(-2*M_PI*M_PI*nu*t);
                            d[k] += rx*cos(M_PI*(x + dx))*sin(M_PI*y)*exp(-2*M_PI*M_PI*nu*(t + dt))/2;
                        } else {
                            wr = w[i + 1 + j*nx];
                        }
                    ...
                    if (isDomain) {
                        if (isBoundary) {
                            wd = cos(M_PI*x)*sin(M_PI*(y - dy))*exp(-2*M_PI*M_PI*nu*t);
                            d[k] += ry*cos(M_PI*x)*sin(M_PI*(y - dy))*exp(-2*M_PI*M_PI*nu*(t + dt))/2;
                        } else {
                            wd = w[i + (j - 1)*nx];
                        }
                    }
                    ...
                    if (isDomain) {
                        if (isBoundary) {
                            wu = cos(M_PI*x)*sin(M_PI*(y + dy))*exp(-2*M_PI*M_PI*nu*t);
                            d[k] += ry*cos(M_PI*x)*sin(M_PI*(y + dy))*exp(-2*M_PI*M_PI*nu*(t + dt))/2;
                        } else {
                            wu = w[i + (j + 1)*nx];
                        }
