...
using namespace PoiseuilleFlow::EllipticDomain::PeacemanRachford;
...
path Solver::createDirectory() {
    auto dirPath = path(
        format("{}/nu={}, a={}, b={}/dx={}, dy={}, r={}", dir, nu, a, b, dx, dy, r)
    );
...
void Solver::setInitialCondition(
    ...
    for (int i = 0; i < nx; i++) {
        for (int j = 0; j < ny; j++) {
            ...
            if (isDomain) {
                w[i][j] = cos(M_PI*x/4) * cos(M_PI*y/2);
            } else {
                w[i][j] = 0;
            }
        }
            ...
            for (int i = 0; i < nx; i++) {
                ...
                if (isDomain) {
                    if (isBoundary) {
                        ...
                        double wb = cos(M_PI*x/4)*cos(M_PI*y/2)*exp(-5*M_PI*M_PI*nu*t/16);
                        double w1b = cos(M_PI*x/4)*cos(M_PI*y/2)*exp(-5*M_PI*M_PI*nu*(t + dt)/16);
            ...
            for (int j = 0; j < ny; j++) {
                ...
                if (isDomain) {
                    if (isBoundary) {
                        ...
                        vy.push_back(cos(M_PI*x/4)*cos(M_PI*y/2)*exp(-5*M_PI*M_PI*nu*(t + dt)/16));
                    } else {
