...
using namespace PoiseuilleFlow::SteadyState::TriangularDomain::FreeSurface::CG;
...
void Solver::calculateResidualElements(
    ...
    for (int j = 0; j < ny; j++) {
        for (int i = 0; i < nx; i++) {
            ...
            if (isDomain && !isBoundary) {                
                d[k] = -2*dx*dx*M_PI*M_PI*sin(M_PI*x)*cos(M_PI*y);
                
                tie(isDomain, isBoundary) = pointInDomain(x - dx, y);

                if (isBoundary) {
                    d[k] -= sin(M_PI*(x - dx))*cos(M_PI*y);
                }
                ...
                if (j == ny - 1 && i > 0 && i < nx - 1) {
                    d[k] = -dx*dx*M_PI*M_PI*sin(M_PI*x)*cos(M_PI*y);      
                    
                    tie(isDomain, isBoundary) = pointInDomain(x - dx, y);

                    if (isBoundary) {
                        d[k] -= sin(M_PI*(x - dx))*cos(M_PI*y)/2;
                    }
