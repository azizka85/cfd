...
using namespace LinearAdvection::ImplicitEuler;

Solver::Solver(double u, double d, double l, double dx, double a, double endTime, double outputTimeStep, string dir) {
    setU(u);
    setD(d);
    setL(l);
    setDX(dx);
...
path Solver::createDirectory() {
    auto dirPath = path(
        format("{}/U={}, d={}, L={}/dx={}, a={}", dir, u, d, l, dx, a)
    );
...
void Solver::setBoundaryCondition(int nx, vector<double> &C) {
    if (nx > 0) {
        C[0] = 1.;        
    }
}
...
void Solver::solve() {
    ...
    while (t <= endTime) {
        Tridiagonal::solve(al, al1, ac, ac0, ac1, ar, ar0, C, C1);

        setBoundaryCondition(nx, C1);
        updateData(nx, C, C1);
