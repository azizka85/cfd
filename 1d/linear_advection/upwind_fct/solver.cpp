...
using namespace LinearAdvection::Upwind::FCT;
...
void Solver::solve() {
    ...
    while (t <= endTime) {
        ...
        for (int i = 0; i < nx; i++) {
            ...
            double nup = ap*(1 - ap)/2;
            double num = am*(1 - am)/2;

            double s = dp1 < 0 ? -1 : 1;

            double gp = s*max(0., min(s*dm1, min(nup*abs(dp1), s*dp2)));

            s = dm1 < 0 ? -1 : 1;

            double gm = s*max(0., min(s*dm2, min(num*abs(dm1), s*dp1)));
