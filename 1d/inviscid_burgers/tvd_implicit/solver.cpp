...
void Solver::solve() {
    ...
    while (t <= endTime) {
        for (int i = 0; i < nx; i++) {
            ...
            double cp = (q(al + bl) + al + bl)/2;
            double cm = (q(ar + br) - ar - br)/2;

            d[i] = u[i];

            vl[i] = -cp;
            vr[i] = -cm;
            vc[i] = 1 + (gp - gm)/2 + cp + cm;
        ...
        Tridiagonal::solve(vl, vc, vr, v, u);
        Tridiagonal::solve(vl, vc, vr, d, v);

        double sv = v[0] + vl[0]*v[nx-1];
        double su = u[0] + vl[0]*u[nx-1];
        ...
        if (t >= tn) {
            ...
            statistics.push_back({n, t, maxDiff});
