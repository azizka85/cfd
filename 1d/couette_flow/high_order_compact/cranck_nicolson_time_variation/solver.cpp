...
using namespace CouetteFlow::HighOrder::Compact::CranckNicolsonWithTimeVariation;

Solver::Solver(double uTop, double nu, double h, double dy, double r, double b, double endTime, double outputTimeStep, string dir) {
    setUTop(uTop);
    setNU(nu);
    setH(h);
    setDY(dy);
...
path Solver::createDirectory() {
    auto dirPath = path(
        format("{}/nu={}, U={}, H={}/dy={}, r={}, b={}", dir, nu, uTop, h, dy, r, b)
    );
...
void Solver::calculateResidualElements(int ny, double r, vector<double> &u, vector<double> &d) {
    if (ny > 0) {
        ...
        for (int i = 1; i < ny - 1; i++) {
            d[i] = (1 + 6*r)*u[i+1] + (10 - 12*r)*u[i] + (1 + 6*r)*u[i-1];
        }
    }
}
