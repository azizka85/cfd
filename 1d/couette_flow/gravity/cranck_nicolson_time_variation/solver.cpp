...
Solver::Solver(double gx, double nu, double h, double dy, double r, double b, double endTime, double outputTimeStep, string dir) {
    setGx(gx);
    setNU(nu);
    setH(h);
    setDY(dy);
...
path Solver::createDirectory() {
    auto dirPath = path(
        format("{}/nu={}, Gx={}, H={}/dy={}, r={}, b={}", dir, nu, gx, h, dy, r, b)
    );
...
void Solver::calculateResidualElements(int ny, double dt, double r, vector<double> &u, vector<double> &d) {
    if (ny > 0) {
        d[0] = u[0];
        d[ny-1] = u[ny-1];

        for (int i = 1; i < ny - 1; i++) {
            d[i] = r*u[i+1]/2 + (1 - r)*u[i] + r*u[i-1]/2 + gx * dt;
        }
    }
}
...
void Solver::solve() {
    ...
    while (t <= endTime) {
        ...
        calculateResidualElements(ny, dt, cr, u, d);
